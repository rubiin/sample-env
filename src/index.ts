...
const writeCongfigFile = (data: Options) => {
  return fs.writeFileSync(defaultEnvironmentPath, JSON.stringify(data, undefined, 2));
};
...
export const main = () => {
  ...
  const prefixes = allArguments.prefix ?? configFile.prefix;
  ...
  reader.on("line", (line) => {
    if (line.length === 0) { fileStream.write("\n"); }
    else if (line.startsWith("#") && removeComments) { fileStream.write("\n"); }
    else {
      if (prefixes.length > 0) {
        const prefix = prefixes.find(prefix => line.startsWith(prefix));
        if (!prefix) return;
      }
